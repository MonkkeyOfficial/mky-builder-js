...
module.exports = {
    createTempFolder: function(config, src, dest, callback)
    {
        function go()
        {
            fs.copy(src, dest, e => {
    ...
    execute: function(tmpPath, callback, errorCallback, sysErrorCallback)
    {
        ...
        fs.ensureDir(tmpPath, e => {
            ...
            globalConfig.exercices.forEach(function(exerciceFolder) {
                var exoConfig = JSON.parse(fs.readFileSync(path.resolve(path.join(exerciceFolder, globalConfig.configFileName))));

                var tmpExoFolder = path.join(tmpPath, 'f_' + exoConfig.uid.toString());

                module.exports.createTempFolder(exoConfig, exerciceFolder, tmpExoFolder, () =>
                {
                    zip.zip(tmpExoFolder, path.join(tmpPath, '_' + exoConfig.uid.toString()), (s, d) => {
